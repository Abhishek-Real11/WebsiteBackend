...
const createAboutUs = async (req, res) => {
  try {
    ...
    data = await AboutUsFooter.create({
...
const updateAboutUs = async (req, res) => {
  try {
    ...
    let result = await AboutUsFooter.findAll({ where: { id: id } });

    if (isActive != result[0].dataValues.isActive) {
      let data = await AboutUsFooter.update(
        { isActive: isActive },
        { where: { id: id } }
      );
      return res.status(200).send({
...
const deleteAboutUs = async (req, res) => {
  try {
    let result = await AboutUsFooter.findAll({
